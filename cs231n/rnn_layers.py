...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x,pre_h,wh,wx,tmp=cache
    tmp=(1.-tmp**2)*dnext_h # derivate for tanh
    dWx=np.dot(x.T,tmp)
    dWh=np.dot(pre_h.T,tmp)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
        x_batch=np.squeeze(x[:,i,:])
        h0,cache_s=rnn_step_forward(x_batch,h0,Wx,Wh,b)
        h.append(h0)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i,f,o,g=np.hsplit(a,4)# 4h*h into h*h
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    df,di,dg=dnext_c*(prev_c,g,i)
    dprev_c = dnext_c * f
    di,df,do=map(lambda x,y:x*y*(1.-y),[di,df,do],[i,f,o])
    dg=dg*(1.-g**2)
