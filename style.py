...
def content_loss(content_weight, content_current, content_original):
    ...
    - scalar content loss
    """
    # content_current=content_current.view(content_current.size()[1],-1)
    # content_original=content_original.view(content_original.size()[1],-1)
    loss=content_weight*(content_current-content_original).norm()**2
    return loss
...
def gram_matrix(features, normalize=True):
    ...
    tmp=features.transpose(1,2)
    gram=torch.bmm(features,tmp)
    if normalize:
        gram=gram/(C*H*W)
    return gram
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i,index in enumerate(style_layers):
        tmp= style_weights[i]*(gram_matrix(feats[index])-style_targets[i]).norm()**2
        out.append(tmp)
...
def tv_loss(img, tv_weight):
    ...
    # this like jitter but should not transfer the recurrent the right to left
    left=img[:,:,:,:1]
    right=img[:,:,:,:-1]
    tmp_w=torch.cat([left,right],dim=3)
    top=img[:,:,:1,:]
    down=img[:,:,:-1,:]
    tmp_h=torch.cat([top,down],dim=2)
    ltv=tv_weight*((img-tmp_w).norm()**2+(img-tmp_h).norm()**2)
    return ltv
